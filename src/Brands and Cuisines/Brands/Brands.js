...
const Brands = ({ brandData }) => {
  ...
  const [displayedBrands, setDisplayedBrands] = useState(itemsPerPage);
  ...
  const handleShowLess = () => {
    setDisplayedBrands(itemsPerPage);
  };

  const isShowMoreVisible = displayedBrands < brandData?.length;
      ...
      <div className="brands-grid">
        {brandData.slice(0, displayedBrands).map((brand, index) => (
          <div key={index} className="single-brand">
            <a className="br-item-box" href={brand.link}>
              {brand.text}
            </a>
          </div>
        ))}
