...
const Cuisines = ({ cuisineData }) => {
  ...
  const [displayedCuisines, setDisplayedCuisines] = useState(itemsPerPage);

  const handleShowMore = () => {
    setDisplayedCuisines((prevDisplayedCuisines) =>
      Math.min(prevDisplayedCuisines + 50, cuisineData?.length)
    );
  };

  const handleShowLess = () => {
    setDisplayedCuisines(itemsPerPage);
  };

  const isShowMoreVisible = displayedCuisines < cuisineData?.length;
    ...
    <div className="cuisines">
      ...
      <div className="cuisines-grid">
        {cuisineData.slice(0, displayedCuisines).map((cuisine, index) => (
          <div key={index} className="single-cuisine">
            <a className="cus-item-box" href={cuisine.link}>
              {cuisine.text}
            </a>
          </div>
        ))}
